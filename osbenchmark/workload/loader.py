...
class WorkloadProcessorRegistry:
    def __init__(self, cfg):
        self.required_processors = [
            TaskFilterWorkloadProcessor(cfg),
            TestModeWorkloadProcessor(cfg),
        ]
...
def workloads(cfg):
    ...
    return [
        _load_single_workload(cfg, repo, workload_name)
        for workload_name in repo.workload_names
    ]
...
def _load_single_workload(cfg, workload_repository, workload_name):
    try:
        ...
        current_workload = reader.read(
            workload_name,
            workload_repository.workload_file(workload_name),
            workload_dir,
        )
        ...
        raise exceptions.SystemSetupError(
            f"Cannot load workload [{workload_name}]. "
            f"List the available workloads with [{PROGRAM_NAME} list workloads]."
        ) from e
...
def load_workload_plugins(
    ...
    logging.getLogger(__name__).debug(
        "Invoking plugin_reader with name [%s] resolved to path [%s]",
        workload_name,
        workload_plugin_path,
    )
...
class GitWorkloadRepository:
    def __init__(self, cfg, fetch, update, repo_class=repo.BenchmarkRepository):
        ...
        self.repo = repo_class(
            remote_url, workloads_dir, repo_name, "workloads", offline, fetch
        )
        if update:
            if repo_revision:
    ...
    def workload_names(self):
        return filter(
            lambda p: os.path.exists(self.workload_file(p)),
            next(os.walk(self.repo.repo_dir))[1],
        )
...
class SimpleWorkloadRepository:
    ...
    def workload_dir(self, workload_name):
        ...
        ), "Expect provided workload name [%s] to match [%s]" % (
            workload_name,
            self.workload_name,
        )
        ...
        for document_set in corpus.documents:
            if document_set.is_bulk:
                ...
                logging.getLogger(__name__).info(
                    "Resolved data root directory for document corpus [%s] in workload [%s] "
                    "to [%s].",
                    corpus.name,
                    workload.name,
                    data_root,
                ...
                elif not preparator.prepare_bundled_document_set(
                    document_set, data_root[0]
                ):
...
class Decompressor:
    ...
    def decompress(self, archive_path, documents_path, uncompressed_size):
        if uncompressed_size:
            msg = (
                f"Decompressing workload data from [{archive_path}] to [{documents_path}] (resulting size: "
        ...
        if uncompressed_size is not None and extracted_bytes != uncompressed_size:
            raise exceptions.DataError(
                f"[{documents_path}] is corrupt. Extracted [{extracted_bytes}] bytes "
...
class Downloader:
    ...
    def download(self, base_url, target_path, size_in_bytes):
        ...
        try:
            ...
            raise exceptions.DataError(
                f"Could not download [{data_url}] to [{target_path}]."
            ) from e
        ...
        if size_in_bytes is not None and actual_size != size_in_bytes:
            raise exceptions.DataError(
                f"[{target_path}] is corrupt. Downloaded [{actual_size}] bytes "
...
class DocumentSetPreparator:
    ...
    def create_file_offset_table(self, document_file_path, expected_number_of_lines):
        ...
        if lines_read and lines_read != expected_number_of_lines:
            ...
            raise exceptions.DataError(
                f"Data in [{document_file_path}] for workload [{self.workload_name}] are invalid. "
    ...
    def prepare_document_set(self, document_set, data_root):
        ...
        archive_path = (
            os.path.join(data_root, document_set.document_archive)
            if document_set.has_compressed_corpus()
            else None
        )
        while True:
            if self.is_locally_available(doc_path) and self.has_expected_size(
            ...
            if (
                ...
                and self.has_expected_size(
                    archive_path, document_set.compressed_size_in_bytes
                )
            ):
                ...
                try:
                    self.downloader.download(
                        document_set.base_url, target_path, expected_size
                    )
                except exceptions.DataError as e:
                    if (
                        e.message
                        == "Cannot download data because no base URL is provided."
                        and self.is_locally_available(target_path)
                    ):
                        raise exceptions.DataError(
    ...
    def prepare_bundled_document_set(self, document_set, data_root):
        ...
        while True:
            if self.is_locally_available(doc_path):
                if self.has_expected_size(
                    ...
                    self.create_file_offset_table(
                        doc_path, document_set.number_of_lines
                    )
                    return True
                else:
                    raise exceptions.DataError(
            ...
            if document_set.has_compressed_corpus() and self.is_locally_available(
                archive_path
            ):
                if self.has_expected_size(
                    archive_path, document_set.compressed_size_in_bytes
...
class TemplateSource:
    ...
    def __init__(
        self, base_path, template_file_name, source=io.FileSource, fileglobber=glob.glob
    ):
    ...
    def load_template_from_file(self):
        ...
        try:
            base_workload = loader.get_source(
                jinja2.Environment(autoescape=select_autoescape(["html", "xml"])),
...
def default_internal_template_vars(glob_helper=lambda f: [], clock=time.Clock):
    ...
    return {
        "globals": {"now": clock.now(), "glob": glob_helper},
        "filters": {"days_ago": time.days_ago},
    }


def render_template(
    template_source, template_vars=None, template_internal_vars=None, loader=None
):
    macros = [
        """
        {% macro collect(parts) -%}
    ...
    if template_internal_vars:
        for macro_type in template_internal_vars:
            for env_global_key, env_global_value in template_internal_vars[
                macro_type
            ].items():
...
def render_template_from_file(
    template_file_name, template_vars, complete_workload_params=None
):
    ...
    def on_after_load_workload(self, workload):
        ...
        for test_procedure in workload.test_procedures:
            ...
            for task in test_procedure.schedule:
                if self._filter_out_match(task):
                    ...
                    for leaf_task in leafs_to_remove:
                        self.logger.info(
                            "Removing sub-task [%s] from test_procedure [%s] due to task filter.",
                            leaf_task,
                            test_procedure,
                        )
            ...
            for task in tasks_to_remove:
                self.logger.info(
                    "Removing task [%s] from test_procedure [%s] due to task filter.",
                    task,
                    test_procedure,
                )
            ...
            for document_set in corpus.documents:
                ...
                if document_set.is_bulk:
                    ...
                    if document_set.has_compressed_corpus():
                        ...
                        raise exceptions.BenchmarkAssertionError(
                            f"Document corpus [{corpus.name}] has neither compressed "
        ...
        for test_procedure in workload.test_procedures:
            for task in test_procedure.schedule:
                ...
                for leaf_task in task:
                    ...
                    if (
                        leaf_task.warmup_iterations is not None
                        and leaf_task.warmup_iterations > leaf_task.clients
                    ):
                        count = leaf_task.clients
                        if self.logger.isEnabledFor(logging.DEBUG):
                            self.logger.debug(
                                "Resetting warmup iterations to %d for [%s]",
                                count,
                                str(leaf_task),
                            )
                        leaf_task.warmup_iterations = count
                    if (
                        leaf_task.iterations is not None
                        and leaf_task.iterations > leaf_task.clients
                    ):
                        count = leaf_task.clients
                        if self.logger.isEnabledFor(logging.DEBUG):
                            self.logger.debug(
                    ...
                    if (
                        leaf_task.warmup_time_period is not None
                        and leaf_task.warmup_time_period > 0
                    ):
                    ...
                    if leaf_task.target_throughput:
                        ...
                        leaf_task.params[
                            "target-throughput"
                        ] = f"{sys.maxsize} {original_throughput.unit}"
...
class CompleteWorkloadParams:
    def __init__(self, user_specified_workload_params=None):
        ...
        self.user_specified_workload_params = (
            user_specified_workload_params if user_specified_workload_params else {}
        )
    ...
    def read(self, workload_name, workload_spec_file, mapping_dir):
        ...
        try:
            ...
            with open(tmp.name, "wt", encoding="utf-8") as f:
                f.write(rendered)
            self.logger.info(
                "Final rendered workload for '%s' has been written to '%s'.",
                workload_spec_file,
                tmp.name,
            )
            ...
            msg += (
                "The complete workload has been written to '{}' for diagnosis.".format(
                    tmp.name
                )
            )
            ...
            self.logger.exception("Could not load [%s].", workload_spec_file)
            msg = "Could not load '{}'. The complete workload has been written to '{}' for diagnosis.".format(
        ...
        try:
            ...
            raise exceptions.InvalidSyntax(
                "version identifier for workload %s must be numeric but was [%s]"
        ...
        if WorkloadFileReader.MINIMUM_SUPPORTED_TRACK_VERSION > workload_version:
            raise exceptions.BenchmarkError(
                "Workload {} is on version {} but needs to be updated at least to version {} to work with the "
                "current version of Benchmark.".format(
                    workload_name,
                    workload_version,
        ...
        if WorkloadFileReader.MAXIMUM_SUPPORTED_TRACK_VERSION < workload_version:
            raise exceptions.BenchmarkError(
                "Workload {} requires a newer version of Benchmark. "
        ...
        try:
            ...
            raise WorkloadSyntaxError(
                "Workload '{}' is invalid.\n\nError details: {}\nInstance: {}\nPath: {}\nSchema path: {}".format(
                    workload_name,
                    ve.message,
                    json.dumps(ve.instance, indent=4, sort_keys=True),
                    ve.absolute_path,
        ...
        unused_user_defined_workload_params = (
            self.complete_workload_params.unused_user_defined_workload_params()
        )
        if len(unused_user_defined_workload_params) > 0:
            err_msg = (
                ...
                "{}".format(
                    ...
                    "\n".join(
                        opts.bulleted_list_of(sorted(list(self.workload_params.keys())))
                    ),
                    "\n".join(
                        opts.bulleted_list_of(
                            self.complete_workload_params.sorted_workload_defined_params
                        )
                    ),
                )
            )
...
class WorkloadPluginReader:
    ...
    def __init__(
        ...
        workload_processor_registry=None,
    ):
...
class WorkloadSpecificationReader:
    ...
    def __init__(
        ...
        complete_workload_params=None,
        selected_test_procedure=None,
    ...
    def __call__(self, workload_name, workload_specification, mapping_dir):
        ...
        description = self._r(
            workload_specification, "description", mandatory=False, default_value=""
        )

        meta_data = self._r(workload_specification, "meta", mandatory=False)
        indices = [
            self._create_index(idx, mapping_dir)
            for idx in self._r(
                workload_specification, "indices", mandatory=False, default_value=[]
            )
        ]
        data_streams = [
            self._create_data_stream(idx)
            for idx in self._r(
        ...
        templates = [
            ...
            for tpl in self._r(
                workload_specification, "templates", mandatory=False, default_value=[]
            )
        ]
        ...
        corpora = self._create_corpora(
            self._r(
                workload_specification, "corpora", mandatory=False, default_value=[]
            ),
            indices,
            data_streams,
        )
    ...
    def _r(self, root, path, error_ctx=None, mandatory=True, default_value=None):
        ...
        try:
            ...
            if mandatory:
                if error_ctx:
                    self._error(
                        "Mandatory element '%s' is missing in '%s'."
                        % (".".join(path), error_ctx)
                    )
                else:
    ...
    def _create_index_template(self, tpl_spec, mapping_dir):
        ...
        return workload.IndexTemplate(
            name, index_pattern, template_content, delete_matching_indices
        )
    ...
    def _load_template(self, contents, description):
        ...
        try:
            rendered = render_template(
                template_source=contents, template_vars=self.workload_params
            )
    ...
    def _create_corpora(self, corpora_specs, indices, data_streams):
        ...
        for corpus_spec in corpora_specs:
            ...
            default_source_format = self._r(
                corpus_spec,
                "source-format",
                mandatory=False,
                default_value=workload.Documents.SOURCE_FORMAT_BULK,
            )
            default_action_and_meta_data = self._r(
                corpus_spec,
            ...
            for doc_spec in self._r(corpus_spec, "documents"):
                ...
                source_format = self._r(
                    ...
                    "source-format",
                    mandatory=False,
                    default_value=default_source_format,
                )
                ...
                if source_format == workload.Documents.SOURCE_FORMAT_BULK:
                    ...
                    if includes_action_and_meta_data:
                        ...
                        target_type = self._r(
                            ...
                            "target-type",
                            mandatory=False,
                            default_value=corpus_target_type,
                            error_ctx=docs,
                        )
                        ...
                        target_ds = self._r(
                            doc_spec,
                            "target-data-stream",
                            mandatory=len(data_streams) > 0
                            and corpus_target_ds is None,
                        ...
                        if target_idx is None and target_ds is None:
                            raise WorkloadSyntaxError(
                                f"a {'target-index' if len(indices) > 0 else 'target-data-stream'} "
                    ...
                    self._error(
                        "Unknown source-format [%s] in document corpus [%s]."
                        % (source_format, name)
                    )
    ...
    def _create_test_procedures(self, workload_spec):
        ...
        for test_procedure_spec in test_procedure_specs:
            ...
            user_info = self._r(
                test_procedure_spec, "user-info", error_ctx=name, mandatory=False
            )
            ...
            meta_data = self._r(
                test_procedure_spec, "meta", error_ctx=name, mandatory=False
            )
            ...
            selected = (
                number_of_test_procedures == 1 or self.selected_test_procedure == name
            )
            if default and default_test_procedure is not None:
                self._error(
                    "Both '%s' and '%s' are defined as default test_procedures. Please define only one of them as default."
                ...
                for sub_task in task:
                    if sub_task.name in known_task_names:
                        self._error(
                            ...
                            "assign a unique name for each task."
                            % (name, sub_task.name)
                        )
                    else:
    ...
    def _get_test_procedure_specs(self, workload_spec):
        ...
        count_defined = len(
            list(
                filter(
                    lambda e: e is not None, [schedule, test_procedure, test_procedures]
                )
            )
        )
    ...
    def parse_parallel(self, ops_spec, ops, test_procedure_name):
        ...
        completed_by = self._r(
            ops_spec, "completed-by", error_ctx="parallel", mandatory=False
        )
            ...
            for task in tasks:
                if task.completes_parent and not completion_task:
                    ...
                    self._error(
                        ...
                        "'completed-by' but only task is allowed to match."
                        % (test_procedure_name, completed_by)
                    )
    ...
    def parse_task(
        ...
        # raise ValueError("Invalid task specification: %s" % task_spec)
        ...
        if isinstance(op_spec, str) and op_spec in ops:
            ...
            op = self.parse_operation(
                op_spec,
                error_ctx="inline operation in test_procedure %s" % test_procedure_name,
            )

        schedule = self._r(task_spec, "schedule", error_ctx=op.name, mandatory=False)
        task_name = self._r(
            task_spec, "name", error_ctx=op.name, mandatory=False, default_value=op.name
        )
        task = workload.Task(
            name=task_name,
            operation=op,
