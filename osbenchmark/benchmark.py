...
def create_arg_parser():
    ...
    def non_empty_list(arg):
        ...
        if len(lst) < 1:
            raise argparse.ArgumentError(
                argument=None, message="At least one argument required!"
            )
        return lst
        ...
        if v == "bundled":
            ...
            try:
                ...
                raise argparse.ArgumentTypeError(
                    f"must be a positive number or 'bundled' but was {v}"
                )
    ...
    def supported_os_version(v):
        if v:
            ...
            if specified_version < min_os_version:
                raise argparse.ArgumentTypeError(
                    f"must be at least {min_os_version} but was {v}"
                )
        return v
    ...
    def add_workload_source(subparser):
        ...
        workload_source_group.add_argument(
            "--workload-path", help="Define the path to a workload."
        )
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Find out more about Benchmark at {}".format(
    ...
    test_execution_parser = subparsers.add_parser(
        "execute-test", help="Run a benchmark"
    )
    ...
    create_workload_parser = subparsers.add_parser(
        "create-workload", help="Create a Benchmark workload from existing data"
    )
    create_workload_parser.add_argument(
        "--workload", required=True, help="Name of the generated workload"
    )
    create_workload_parser.add_argument(
        "--indices",
        type=non_empty_list,
    ...
    install_parser = subparsers.add_parser(
        "install", help="Installs an OpenSearch node locally"
    )
    ...
    install_parser.add_argument(
        "--build-type", help=argparse.SUPPRESS, choices=["tar", "docker"], default="tar"
    )
    install_parser.add_argument(
        "--provision-config-repository",
        help="Define the repository from where Benchmark will load provision_configs and provision_config_instances (default: default).",
    ...
    install_parser.add_argument(
        "--http-port", help="The port to expose for HTTP traffic", default="39200"
    )
    install_parser.add_argument(
        "--node-name",
    ...
    start_parser = subparsers.add_parser(
        "start", help="Starts an OpenSearch node locally"
    )
    ...
    for p in [
        ...
        stop_parser,
        info_parser,
        create_workload_parser,
    ]:
        # This option is needed to support a separate configuration for the integration tests on the same machine
        p.add_argument("--configuration-name", help=argparse.SUPPRESS, default=None)
        p.add_argument(
            "--quiet",
...
def print_help_on_errors():
    ...
    console.println(
        f"* Ask a question on the forum at {console.format.link(FORUM_LINK)}."
    )
    console.println(
        f"* Raise an issue at {console.format.link('https://github.com/opensearch-project/OpenSearch-Benchmark/issues')} "
...
def execute_test(cfg, kill_running_processes=False):
    ...
    if kill_running_processes:
        ...
        if other_benchmark_processes:
            ...
            msg = (
                f"There are other Benchmark processes running on this machine (PIDs: {pids}) but only one Benchmark "
        ...
        if not already_running:
            ...
            while not shutdown_complete and times_interrupted < 2:
                try:
                    ...
                    console.info(
                        "Please wait a moment for Benchmark's internal components to shutdown."
                    )
            if not shutdown_complete and times_interrupted > 0:
                ...
                console.println("")
                console.warn(
                    "The next test_execution may fail due to an unclean shutdown."
                )
                console.println("")
                ...
                console.warn(
                    "Could not terminate all internal processes within timeout. Please check and force-terminate "
...
def configure_workload_params(arg_parser, args, cfg, command_requires_workload=True):
    ...
    if args.workload_path:
        cfg.add(
            ...
            "workload",
            "workload.path",
            os.path.abspath(io.normalize_path(args.workload_path)),
        )
        ...
        if args.workload_revision:
            ...
            arg_parser.error(
                "argument --workload-revision not allowed with argument --workload-path"
            )
        if command_requires_workload and args.workload:
            # stay as close as possible to argparse errors although we have a custom validation.
            arg_parser.error(
                "argument --workload not allowed with argument --workload-path"
            )
    else:
        cfg.add(
            config.Scope.applicationOverride,
    ...
    if command_requires_workload:
        cfg.add(
            ...
            "workload",
            "params",
            opts.to_dict(args.workload_params),
        )
        cfg.add(
            config.Scope.applicationOverride,
            "workload",
...
def configure_builder_params(
    args, cfg, command_requires_provision_config_instance=True
):
    if args.provision_config_path:
        cfg.add(
            config.Scope.applicationOverride,
            "builder",
            "provision_config.path",
            os.path.abspath(io.normalize_path(args.provision_config_path)),
        )
        cfg.add(config.Scope.applicationOverride, "builder", "repository.name", None)
        cfg.add(
            config.Scope.applicationOverride, "builder", "repository.revision", None
        )
    else:
        cfg.add(
            config.Scope.applicationOverride,
            "builder",
            "repository.name",
            args.provision_config_repository,
        )
        cfg.add(
            config.Scope.applicationOverride,
            "builder",
            "repository.revision",
            args.provision_config_revision,
        )
    ...
    if command_requires_provision_config_instance:
        ...
        cfg.add(
            ...
            "builder",
            "distribution.repository",
            args.distribution_repository,
        )
        cfg.add(
            config.Scope.applicationOverride,
            "builder",
            "provision_config_instance.names",
            opts.csv_to_list(args.provision_config_instance),
        )
        cfg.add(
            config.Scope.applicationOverride,
            "builder",
            "provision_config_instance.params",
            opts.to_dict(args.provision_config_instance_params),
        )
    ...
    try:
        if sub_command == "compare":
            ...
            cfg.add(
                config.Scope.applicationOverride,
                "system",
                "list.test_executions.max_results",
                args.limit,
            )
            configure_builder_params(
                args, cfg, command_requires_provision_config_instance=False
            )
            configure_workload_params(
                arg_parser, args, cfg, command_requires_workload=False
            )
            dispatch_list(cfg)
        elif sub_command == "download":
            cfg.add(
                config.Scope.applicationOverride, "builder", "target.os", args.target_os
            )
            cfg.add(
                config.Scope.applicationOverride,
                "builder",
            ...
            cfg.add(
                config.Scope.applicationOverride, "builder", "node.name", args.node_name
            )
            cfg.add(
                config.Scope.applicationOverride,
                "builder",
                "master.nodes",
                opts.csv_to_list(args.master_nodes),
            )
            cfg.add(
                config.Scope.applicationOverride,
                "builder",
                "seed.hosts",
                opts.csv_to_list(args.seed_hosts),
            )
            cfg.add(
                config.Scope.applicationOverride,
                "builder",
                "provision_config_instance.plugins",
                opts.csv_to_list(args.opensearch_plugins),
            )
            cfg.add(
                config.Scope.applicationOverride,
                "builder",
            ...
            cfg.add(
                ...
                "worker_coordinator",
                "profiling",
                args.enable_worker_coordinator_profiling,
            )
            cfg.add(
                config.Scope.applicationOverride,
                "worker_coordinator",
                "assertions",
                args.enable_assertions,
            )
            cfg.add(
                config.Scope.applicationOverride,
                "worker_coordinator",
            ...
            cfg.add(
                ...
                "builder",
                "skip.rest.api.check",
                convert.to_bool(args.skip_rest_api_check),
            )
            ...
            cfg.add(
                config.Scope.applicationOverride, "generator", "indices", args.indices
            )
            cfg.add(
                config.Scope.applicationOverride,
                "generator",
        ...
        logging.getLogger(__name__).exception(
            "Cannot run subcommand [%s].", sub_command
        )
        msg = str(e.message)
        nesting = 0
        ...
        logging.getLogger(__name__).exception(
            "A fatal error occurred while running subcommand [%s].", sub_command
        )
...
def main():
    ...
    cfg.add(
        config.Scope.application, "system", "time.start", datetime.datetime.utcnow()
    )
    ...
    net.init()
    if not args.offline:
        probing_url = cfg.opts(
            "system", "probing.url", default_value="https://github.com", mandatory=False
        )
        if not net.has_internet_connection(probing_url):
            console.warn(
                "No Internet connection detected. Automatic download of workload data sets etc. is disabled.",
                logger=logger,
            )
    ...
    if success:
        console.println("")
        console.info(
            "SUCCESS (took %d seconds)" % (end - start), overline="-", underline="-"
        )
    else:
        console.println("")
        console.info(
            "FAILURE (took %d seconds)" % (end - start), overline="-", underline="-"
        )
        sys.exit(64)
