...
class WorkloadRandomizationTests(TestCase):
        ...
        def __init__(self):
            ...
            self.saved_values = {
                self.op_name_1: {
                    ...
                    self.field_name_2: {"lte": "06/06/2016",
                                        "gte": "05/05/2016", "format": "dd/MM/yyyy"}
                },
                self.op_name_2: {
            ...
            self.new_values = {
                self.op_name_1: {
                    ...
                    self.field_name_2: {"lte": "04/04/2016",
                                        "gte": "03/03/2016", "format": "dd/MM/yyyy"}
                },
                self.op_name_2: {
    ...
    def test_range_finding_function(self):
        ...
        single_range_query_expected = [
            ("trip_distance", ["bool", "filter", "range"])]
        self.assertEqual(single_range_query_result,
                         single_range_query_expected)
        ...
        self.assertEqual(multiple_nested_range_query_result,
                         multiple_nested_range_query_expected)
    ...
    def test_get_randomized_values(self):
        ...
        for rf, expected_values_dict in zip([1.0, 0.0], [helper.saved_values, helper.new_values]):
            ...
            modified_range_2 = modified_params["body"]["query"]["bool"][
                "filter"]["must"][0]["range"][helper.field_name_2]
            self.assertEqual(
                modified_range_1["lt"], expected_values_dict[helper.op_name_1][helper.field_name_1]["lte"])
            # Note it should keep whichever of lt/lte it found in the original query
            self.assertEqual(
                modified_range_1["gte"], expected_values_dict[helper.op_name_1][helper.field_name_1]["gte"])

            self.assertEqual(
                modified_range_2["lte"], expected_values_dict[helper.op_name_1][helper.field_name_2]["lte"])
            self.assertEqual(
                modified_range_2["gte"], expected_values_dict[helper.op_name_1][helper.field_name_2]["gte"])
            self.assertEqual(
                modified_range_2["format"], expected_values_dict[helper.op_name_1][helper.field_name_2]["format"])
            ...
            self.assertEqual(
                modified_range_1["lt"], expected_values_dict[helper.op_name_2][helper.field_name_1]["lte"])
            self.assertEqual(
                modified_range_1["gte"], expected_values_dict[helper.op_name_2][helper.field_name_1]["gte"])
            self.assertEqual(modified_params["index"], helper.index_name)
    ...
    def test_on_after_load_workload(self):
        ...
        self.assertEqual(
            processor.N, loader.QueryRandomizerWorkloadProcessor.DEFAULT_N)
        self.assertEqual(type(processor.N), int)
        self.assertEqual(
            processor.rf, loader.QueryRandomizerWorkloadProcessor.DEFAULT_RF)
