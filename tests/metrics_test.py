...
class EsClientTests(TestCase):
    ...
    def test_config_opts_parsing(self, client_esclientfactory):
        ...
        cfg.add(config.Scope.applicationOverride, "publishing_results", "datastore.host", _datastore_host)
        cfg.add(config.Scope.applicationOverride, "publishing_results", "datastore.port", _datastore_port)
        cfg.add(config.Scope.applicationOverride, "publishing_results", "datastore.secure", _datastore_secure)
        cfg.add(config.Scope.applicationOverride, "publishing_results", "datastore.user", _datastore_user)
        cfg.add(config.Scope.applicationOverride, "publishing_results", "datastore.password", _datastore_password)
        if not _datastore_verify_certs:
            cfg.add(config.Scope.applicationOverride, "publishing_results", "datastore.ssl.verification_mode", "none")
...
class StatsCalculatorTests(TestCase):
    def test_calculate_global_stats(self):
        ...
        cfg.add(config.Scope.application, "publishing_results", "datastore.type", "in-memory")
...
class GlobalStatsCalculatorTests(TestCase):
    ...
    def test_add_administrative_task_with_error_rate_in_report(self):
        op = Operation(name='delete-index', operation_type='DeleteIndex', params={'include-in-publishing-results': False})
