...
class DistributionRepositoryTests(TestCase):
    ...
    def test_release_repo_config_with_default_url(self, os_name, cpu_arch):
        ...
        self.assertEqual("https://artifacts.opensearch.org/\
            releases/bundle/opensearch/1.0.0/opensearch-1.0.0-linux-x64.tar.gz", repo.download_url)
    ...
    def test_release_repo_config_with_user_url(self):
        renderer = supplier.TemplateRenderer(version="1.0.0")
        repo = supplier.DistributionRepository(name="release", distribution_config={
            "jdk.unbundled.release_url": "https://artifacts.opensearch.org/\
                releases/bundle/opensearch/{{VERSION}}/opensearch-{{VERSION}}-{{OSNAME}}-{{ARCH}}.tar.gz",
            "runtime.jdk.bundled": "false",
            # user override
            "release.url": "https://artifacts.opensearch.org/\
                releases/bundle/opensearch/{{VERSION}}/opensearch-{{VERSION}}-{{OSNAME}}-{{ARCH}}.tar.gz",
            "release.cache": "false"
        }, template_renderer=renderer)
        self.assertEqual("https://artifacts.opensearch.org/\
    ...
    def test_missing_cache(self):
        renderer = supplier.TemplateRenderer(version="1.0.0")
        repo = supplier.DistributionRepository(name="release", distribution_config={
            "jdk.unbundled.release.url": "https://artifacts.opensearch.org/\
                releases/bundle/opensearch/{{VERSION}}/opensearch-{{VERSION}}-{{OSNAME}}-{{ARCH}}.tar.gz",
            "runtime.jdk.bundled": "false"
        }, template_renderer=renderer)
        with self.assertRaises(exceptions.SystemSetupError) as ctx:
