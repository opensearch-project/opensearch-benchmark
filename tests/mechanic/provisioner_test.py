...
class BareProvisionerTests(TestCase):
    ...
    def test_prepare_without_plugins(self, mock_rm, mock_ensure_dir, mock_decompress):
        ...
        installer = provisioner.ElasticsearchInstaller(provision_config=
        team.ProvisionConfig(
            names="unit-test-provision_config",
            root_path=None,
            config_paths=[HOME_DIR + "/.rally/benchmarks/teams/default/my-provision_config"],
        ...
        self.assertEqual("8", node_config.provision_config_runtime_jdks)
        ...
        self.assertEqual(HOME_DIR + "/.rally/benchmarks/teams/default/my-provision_config", source_root_path)
...
class ElasticsearchInstallerTests(TestCase):
    ...
    def test_prepare_default_data_paths(self, mock_rm, mock_ensure_dir, mock_decompress):
        installer = provisioner.ElasticsearchInstaller(provision_config=team.ProvisionConfig(names="defaults",
                                                                    root_path=None,
                                                                    config_paths="/tmp"),
                                                       java_home="/usr/local/javas/java8",
                                                       node_name="rally-node-0",
...
class DockerProvisionerTests(TestCase):
    ...
    def test_provisioning_with_defaults(self, uuid4):
        ...
        c = team.ProvisionConfig("unit-test-provision_config", None, "/tmp", variables={
            "docker_image": "docker.elastic.co/elasticsearch/elasticsearch-oss"
        })

        docker = provisioner.DockerProvisioner(provision_config=c,
                                               node_name="rally-node-0",
                                               ip="10.17.22.33",
                                               http_port=39200,
