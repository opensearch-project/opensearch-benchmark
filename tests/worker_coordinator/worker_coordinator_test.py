...
class WorkerCoordinatorTests(TestCase):
    ...
    class StaticClientFactory:
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.osearch = WorkerCoordinatorTests.StaticClientFactory.PATCHER.start()
            self.osearch.indices.stats.return_value = {"mocked": True}
...
class AsyncExecutorTests(TestCase):
    ...
    @run_async
    async def test_execute_schedule_in_throughput_mode(self, osearch):
        task_start = time.perf_counter()
        osearch.new_request_context.return_value = AsyncExecutorTests.StaticRequestTiming(task_start=task_start)

        osearch.bulk.return_value = as_future(io.StringIO('{"errors": false, "took": 8}'))
    ...
    @mock.patch("elasticsearch.Elasticsearch")
    @run_async
    async def test_execute_schedule_with_progress_determined_by_runner(self, osearch):
        task_start = time.perf_counter()
        osearch.new_request_context.return_value = AsyncExecutorTests.StaticRequestTiming(task_start=task_start)
    ...
    @mock.patch("elasticsearch.Elasticsearch")
    @run_async
    async def test_execute_schedule_runner_overrides_times(self, osearch):
        task_start = time.perf_counter()
        osearch.new_request_context.return_value = AsyncExecutorTests.StaticRequestTiming(task_start=task_start)
    ...
    @mock.patch("elasticsearch.Elasticsearch")
    @run_async
    async def test_execute_schedule_throughput_throttled(self, osearch):
        def perform_request(*args, **kwargs):
            return as_future()

        osearch.init_request_context.return_value = {
            "request_start": 0,
            "request_end": 10
        }
        ...
        osearch.transport.perform_request.side_effect = perform_request
    ...
    @mock.patch("elasticsearch.Elasticsearch")
    @run_async
    async def test_cancel_execute_schedule(self, osearch):
        osearch.init_request_context.return_value = {
            "request_start": 0,
            "request_end": 10
        }
    ...
    @mock.patch("elasticsearch.Elasticsearch")
    @run_async
    async def test_execute_schedule_aborts_on_error(self, osearch):
        ...
        for on_error in ["abort", "continue"]:
            with self.subTest():
                ...
                with self.assertRaises(exceptions.BenchmarkAssertionError) as ctx:
                    await worker_coordinator.execute_single(self.context_managed(runner), osearch, params, on_error=on_error)
                self.assertEqual(
    ...
    async def test_execute_single_with_http_400_aborts_when_specified(self):
        ...
        with self.assertRaises(exceptions.BenchmarkAssertionError) as ctx:
            await worker_coordinator.execute_single(self.context_managed(runner), osearch, params, on_error="abort")
        self.assertEqual(
    ...
    async def test_execute_single_with_http_400(self):
        ...
        ops, unit, request_meta_data = await worker_coordinator.execute_single(
            self.context_managed(runner), osearch, params, on_error="continue")
    ...
    async def test_execute_single_with_key_error(self):
        ...
        with self.assertRaises(exceptions.SystemSetupError) as ctx:
            await worker_coordinator.execute_single(self.context_managed(runner), osearch, params, on_error="continue")
        self.assertEqual(
