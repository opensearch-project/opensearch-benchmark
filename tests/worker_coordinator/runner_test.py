...
# A dummy producer that simply records messages that are "sent"
class DummyProducer:
    def __init__(self):
        self.sent_messages = []
...
@pytest.mark.asyncio
async def test_produce_stream_message_multiple_messages():
    produce_runner = runner.ProduceStreamMessage()
    dummy_producer = DummyProducer()

    message_body = "Message 1\n\nMessage 2\n   \nMessage 3"
    params = {
        "message-producer": dummy_producer,
        "topic": "test-topic",
        "body": message_body
    }
    result = await produce_runner(None, params)
    # Expected messages are "Message 1", "Message 2", and "Message 3" (ignoring empty lines).
    assert result == {"weight": 3, "unit": "ops", "success": True}
    assert dummy_producer.sent_messages == [
        ("test-topic", "Message 1"),
        ("test-topic", "Message 2"),
        ("test-topic", "Message 3")
    ]

@pytest.mark.asyncio
async def test_produce_stream_message_exception():
    produce_runner = runner.ProduceStreamMessage()
    ...
    params = {
        "message-producer": failing_producer,
        "topic": "test-topic",
        "body": "Test message"
    }
    with pytest.raises(exceptions.BenchmarkError) as excinfo:
        await produce_runner(None, params)
    assert "Failed to produce message to Kafka: Simulated failure" in str(excinfo.value)