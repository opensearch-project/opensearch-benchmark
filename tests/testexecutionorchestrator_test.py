...
from esrally import config, exceptions, testexecutionorchestrator
...
def benchmark_only_pipeline():
    ...
    original = testexecutionorchestrator.pipelines[test_pipeline_name]
    pipeline = testexecutionorchestrator.Pipeline(test_pipeline_name, "Pipeline intended for unit-testing", mock.Mock())
    yield pipeline
    # restore prior pipeline!
    testexecutionorchestrator.pipelines[test_pipeline_name] = original


@pytest.fixture
def unittest_pipeline():
    pipeline = testexecutionorchestrator.Pipeline("unit-test-pipeline", "Pipeline intended for unit-testing", mock.Mock())
    yield pipeline
    del testexecutionorchestrator.pipelines[pipeline.name]
...
def test_finds_available_pipelines():
    ...
    assert expected == testexecutionorchestrator.available_pipelines()
...
def test_prevents_running_an_unknown_pipeline():
    ...
    with pytest.raises(
        ...
        testexecutionorchestrator.run(cfg)
