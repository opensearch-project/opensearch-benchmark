...
def test_docker_geonames():
    test_command = "test_execution --pipeline=benchmark-only --test-mode --track=geonames " \
...
def test_docker_override_cmd():
    test_command = "esrally test_execution --pipeline=benchmark-only --test-mode --track=geonames " \
                   "--challenge=append-no-conflicts-index-only --target-hosts=es01:9200"
    run_docker_compose_test(test_command)
