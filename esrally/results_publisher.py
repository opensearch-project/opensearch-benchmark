...
def write_single_results(results_file, results_format, cwd, numbers_align, headers, data_plain, data_rich):
    if results_format == "markdown":
        ...
        raise exceptions.SystemSetupError("Unknown results format '%s'" % results_format)
...
class Summaryresultser:
    def __init__(self, results, config):
        self.results = results
        self.results_file = config.opts("resultsing", "output.path")
        self.results_format = config.opts("resultsing", "format")
        self.numbers_align = config.opts("resultsing", "numbers.align",
                                         mandatory=False, default_value="right")
        resultsing_values = config.opts("resultsing", "values")
        self.results_all_values = resultsing_values == "all"
        self.results_all_percentile_values = resultsing_values == "all-percentiles"
        self.show_processing_time = convert.to_bool(config.opts("resultsing", "output.processingtime",
    ...
    def results(self):
        ...
        warnings = []
        metrics_table = []
        metrics_table.extend(self._results_totals(stats))
        metrics_table.extend(self._results_ml_processing_times(stats))

        metrics_table.extend(self._results_gc_metrics(stats))

        metrics_table.extend(self._results_disk_usage(stats))
        metrics_table.extend(self._results_segment_memory(stats))
        metrics_table.extend(self._results_segment_counts(stats))

        metrics_table.extend(self._results_transform_stats(stats))

        for record in stats.op_metrics:
            task = record["task"]
            metrics_table.extend(self._results_throughput(record, task))
            metrics_table.extend(self._results_latency(record, task))
            metrics_table.extend(self._results_service_time(record, task))
            # this is mostly needed for debugging purposes but not so relevant to end users
            if self.show_processing_time:
                metrics_table.extend(self._results_processing_time(record, task))
            metrics_table.extend(self._results_error_rate(record, task))
    ...
    def _results_throughput(self, values, task):
    ...
    def _results_latency(self, values, task):
        return self._results_percentiles("latency", task, values["latency"])

    def _results_service_time(self, values, task):
        return self._results_percentiles("service time", task, values["service_time"])

    def _results_processing_time(self, values, task):
        return self._results_percentiles("processing time", task, values["processing_time"])

    def _results_percentiles(self, name, task, value):
        lines = []
        if value:
            for percentile in metrics.percentiles_for_sample_size(sys.maxsize):
                ...
                a_line = self._line("%sth percentile %s" % (percentile, name), task, percentile_value, "ms",
                                    force=self.results_all_percentile_values)
                self._append_non_empty(lines, a_line)
        return lines

    def _results_error_rate(self, values, task):
        return self._join(
            self._line("error rate", task, values["error_rate"], "%", lambda v: "%.2f" % (v * 100.0))
        )

    def _results_totals(self, stats):
        lines = []
        lines.extend(self._results_total_time("indexing time", stats.total_time))
        lines.extend(self._results_total_time_per_shard("indexing time", stats.total_time_per_shard))
        lines.extend(self._results_total_time("indexing throttle time", stats.indexing_throttle_time))
        lines.extend(self._results_total_time_per_shard("indexing throttle time", stats.indexing_throttle_time_per_shard))
        lines.extend(self._results_total_time("merge time", stats.merge_time))
        lines.extend(self._results_total_count("merge count", stats.merge_count))
        lines.extend(self._results_total_time_per_shard("merge time", stats.merge_time_per_shard))
        lines.extend(self._results_total_time("merge throttle time", stats.merge_throttle_time))
        lines.extend(self._results_total_time_per_shard("merge throttle time", stats.merge_throttle_time_per_shard))
        lines.extend(self._results_total_time("refresh time", stats.refresh_time))
        lines.extend(self._results_total_count("refresh count", stats.refresh_count))
        lines.extend(self._results_total_time_per_shard("refresh time", stats.refresh_time_per_shard))
        lines.extend(self._results_total_time("flush time", stats.flush_time))
        lines.extend(self._results_total_count("flush count", stats.flush_count))
        lines.extend(self._results_total_time_per_shard("flush time", stats.flush_time_per_shard))
        return lines

    def _results_total_time(self, name, total_time):
        unit = "min"
        return self._join(
            self._line("Cumulative {} of primary shards".format(name), "", total_time, unit, convert.ms_to_minutes),
        )

    def _results_total_time_per_shard(self, name, total_time_per_shard):
        unit = "min"
        return self._join(
    ...
    def _results_total_count(self, name, total_count):
        return self._join(
            self._line("Cumulative {} of primary shards".format(name), "", total_count, ""),
        )

    def _results_ml_processing_times(self, stats):
        lines = []
    ...
    def _results_gc_metrics(self, stats):
        return self._join(
    ...
    def _results_disk_usage(self, stats):
        return self._join(
    ...
    def _results_segment_memory(self, stats):
        unit = "MB"
        return self._join(
    ...
    def _results_segment_counts(self, stats):
        return self._join(
            self._line("Segment count", "", stats.segment_count, "")
        )

    def _results_transform_stats(self, stats):
        lines = []
    ...
    def _line(self, k, task, v, unit, converter=lambda x: x, force=False):
        if v is not None or force or self.results_all_values:
...
class Comparisonresultser:
    ...
    def _metrics_table(self, baseline_stats, contender_stats, plain):
        self.plain = plain
        metrics_table = []
        metrics_table.extend(self._results_total_times(baseline_stats, contender_stats))
        metrics_table.extend(self._results_ml_processing_times(baseline_stats, contender_stats))
        metrics_table.extend(self._results_gc_metrics(baseline_stats, contender_stats))
        metrics_table.extend(self._results_disk_usage(baseline_stats, contender_stats))
        metrics_table.extend(self._results_segment_memory(baseline_stats, contender_stats))
        metrics_table.extend(self._results_segment_counts(baseline_stats, contender_stats))
        metrics_table.extend(self._results_transform_processing_times(baseline_stats, contender_stats))

        for t in baseline_stats.tasks():
            if t in contender_stats.tasks():
                metrics_table.extend(self._results_throughput(baseline_stats, contender_stats, t))
                metrics_table.extend(self._results_latency(baseline_stats, contender_stats, t))
                metrics_table.extend(self._results_service_time(baseline_stats, contender_stats, t))
                if self.show_processing_time:
                    metrics_table.extend(self._results_processing_time(baseline_stats, contender_stats, t))
                metrics_table.extend(self._results_error_rate(baseline_stats, contender_stats, t))
    ...
    def _results_throughput(self, baseline_stats, contender_stats, task):
    ...
    def _results_latency(self, baseline_stats, contender_stats, task):
        baseline_latency = baseline_stats.metrics(task)["latency"]
        contender_latency = contender_stats.metrics(task)["latency"]
        return self._results_percentiles("latency", task, baseline_latency, contender_latency)

    def _results_service_time(self, baseline_stats, contender_stats, task):
        ...
        return self._results_percentiles("service time", task, baseline_service_time, contender_service_time)

    def _results_processing_time(self, baseline_stats, contender_stats, task):
        ...
        return self._results_percentiles("processing time", task, baseline_processing_time, contender_processing_time)

    def _results_percentiles(self, name, task, baseline_values, contender_values):
        lines = []
        for percentile in metrics.percentiles_for_sample_size(sys.maxsize):
    ...
    def _results_error_rate(self, baseline_stats, contender_stats, task):
    ...
    def _results_ml_processing_times(self, baseline_stats, contender_stats):
        lines = []
    ...
    def _results_transform_processing_times(self, baseline_stats, contender_stats):
        lines = []
    ...
    def _results_total_times(self, baseline_stats, contender_stats):
        lines = []
        lines.extend(self._results_total_time(
            "indexing time",
            baseline_stats.total_time, contender_stats.total_time
        ))
        lines.extend(self._results_total_time_per_shard(
            "indexing time",
        ...
        lines.extend(self._results_total_count(
            "merge count",
    ...
    def _results_total_time(self, name, baseline_total, contender_total):
        unit = "min"
        return self._join(
    ...
    def _results_total_time_per_shard(self, name, baseline_per_shard, contender_per_shard):
        unit = "min"
        return self._join(
    ...
    def _results_total_count(self, name, baseline_total, contender_total):
        return self._join(
            self._line("Cumulative {} of primary shards".format(name), baseline_total, contender_total, "", "",
                       treat_increase_as_improvement=False)
        )

    def _results_gc_metrics(self, baseline_stats, contender_stats):
        return self._join(
    ...
    def _results_disk_usage(self, baseline_stats, contender_stats):
        return self._join(
    ...
    def _results_segment_memory(self, baseline_stats, contender_stats):
        return self._join(
    ...
    def _results_segment_counts(self, baseline_stats, contender_stats):
        return self._join(
