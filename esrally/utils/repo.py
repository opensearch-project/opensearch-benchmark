...
class RallyRepository:
    ...
    def __init__(self, remote_url, root_dir, repo_name, resource_name, offline, fetch=True):
        ...
        default = "opensearch-benchmark-provisionconfigs"
        self.url = remote_url
        self.remote = self.url is not None and self.url != default and self.url.strip() != ""
        ...
        if self.remote and not self.offline and fetch:
            ...
            if not git.is_working_copy(self.repo_dir):    
                if io.exists(self.repo_dir) and repo_name != "default":
                    ...
                    self.logger.info("Proceeding forward with no provided ProvisionConfigs repository.")
    ...
    def useOpensearchBenchmarkProvisionConfigs(self):
        self.logger.info("Using default opensearch-benchmark-provisionconfigs directory within repository")
        root_dir = os.getcwd().split("/esrally")[0]
        return os.path.join(root_dir, "opensearch-benchmark-provisionconfigs")
