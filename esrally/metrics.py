...
class EsClientFactory:
    ...
    def __init__(self, cfg):
        self._config = cfg
        host = self._config.opts("publishing_results", "datastore.host")
        port = self._config.opts("publishing_results", "datastore.port")
        secure = convert.to_bool(self._config.opts("publishing_results", "datastore.secure"))
        user = self._config.opts("publishing_results", "datastore.user")
        password = self._config.opts("publishing_results", "datastore.password")
        verify = self._config.opts("publishing_results", "datastore.ssl.verification_mode", default_value="full", mandatory=False) != "none"
        ca_path = self._config.opts("publishing_results", "datastore.ssl.certificate_authorities", default_value=None, mandatory=False)
        self.probe_version = self._config.opts("publishing_results", "datastore.probe.cluster_version", default_value=True, mandatory=False)
...
def metrics_store_class(cfg):
    if cfg.opts("publishing_results", "datastore.type") == "elasticsearch":
...
class EsResultsStore:
    """
    Stores the results of a race in a format that is better suited for publishing results with Kibana.
    """
    ...
    def __call__(self):
        ...
        for tasks in self.challenge.schedule:
            for task in tasks:
                ...
                if task.operation.include_in_publishing_results or error_rate > 0:
