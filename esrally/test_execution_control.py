...
class BenchmarkActor(actor.RallyActor):
    ...
    @actor.no_retry("test_execution control")  # pylint: disable=no-value-for-parameter
...
def test_execution(cfg, sources=False, distribution=False, external=False, docker=False):
    ...
    try:
        ...
        logger.info("User has cancelled the benchmark (detected by test_execution control).")
        # notify the coordinator so it can properly handle this state. Do it blocking so we don't have a test_execution between this message
...
def run(cfg):
    ...
    logger.info("Race id [%s]", test_execution_id)
    if len(name) == 0:
